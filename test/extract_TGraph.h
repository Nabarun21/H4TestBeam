...
void  extract_shift_Tgraph( vector<float> hodo, vector<float> calo, double scale ,double shiftx,double shifty,int k,TH2F* Mg[2],int s ,TString run_num){
...
if((4<k) &&(k<9))
{
 for (int j=0 ;j<hodo.size();j++)
    {
      Mg[k-1]->Fill( (hodo.at(j))+shifty,(calo.at(j)/scale)-hodo.at(j)-shifty);
...
for (int j=0 ;j<hodo.size();j++)
    {
      Mg[k-1]->Fill( (hodo.at(j)+shiftx),(calo.at(j)/scale)-hodo.at(j)-shiftx);
